...
(function(){
  // Detect we are on the product page by looking for the main container
  const main = document.getElementById('product-main');
  if (!main) {
    // Not on product.html — do nothing
    return;
  }

  // Elements (may or may not exist depending on your markup)
  ...
  function safe(el, fn){
    if (el) try { fn(); } catch(e){ console.warn(e); }
  }
  ...
  safe(thumbWrap, () => {
    thumbWrap.addEventListener('click', (e)=>{
      ...
      if (!t) return;
      setBig(t.getAttribute('data-src'));
  ...
  // Quantity controls
  safe(qtyMinus, ()=> qtyMinus.addEventListener('click', ()=>{
    const v = Number(qtyInput?.value || 1);
    if (qtyInput) qtyInput.value = Math.max(1, v - 1);
  }));
  safe(qtyPlus,  ()=> qtyPlus.addEventListener('click', ()=>{
    const v = Number(qtyInput?.value || 1);
  ...
  // Add to cart
  safe(addBtn, ()=> addBtn.addEventListener('click', ()=>{
    ...
    // Optional: update cart count badge if present
